...
[![Deploy](https://github.com/elchika-inc/ui/actions/workflows/deploy.yml/badge.svg?branch=main)](https://github.com/elchika-inc/ui/actions/workflows/deploy.yml)
[![standards](https://img.shields.io/badge/standards-2026--08--27_(rev.82)-blue)](https://github.com/elchika-inc/standards/blob/main/CHANGELOG.md)
...
- `dashboard-01` はサイドバー・カード・チャートを含む dashboard の枠。上流の `data-table.tsx` は npm 依存 6 件を必要とするため配布せず、data-table 専用の 6 依存を追加しない。
- `dashboard-table` は除外した table の機能構成を参照し、既存の registry 部品と React state だけで自作した block。列ヘッダによるソート・絞り込み・列表示・行選択・詳細 drawer・chart を備えるが、DnD による行順の変更は搭載しない。
