...
5. `provenance.json`（`PROVENANCE_DATE=$(date +%F) PROVENANCE_MODIFIED='{"<name>":"実際の変更内容"}' node scripts/record-provenance.mjs` で自動記録される。`modified` は自動生成せず、未指定なら停止する）。自作 component は `--original` で記録する。
