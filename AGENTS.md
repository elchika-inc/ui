...
- **PR は base に追随させてからマージする（rebase は使わない）**。ruleset の `strict_required_status_checks_policy` が true なので、base が進むと head が古い PR はマージを拒否される（並行 PR の 2 本目以降は必ず該当する。2026-09-04 の PR #50 で "the head branch is not up to date with the base branch" を実測）。追随は `gh pr update-branch <番号>`（GitHub 側で main をブランチへ merge。既存コミットの SHA は不変）か、worker への `git merge origin/main` の指示で行う。**rebase で追随しない** — 実装コミットの SHA が変わり、`.docs/reviews/` の `verified_impl_sha` が履歴から消えて `check-evidence` の祖先判定を通らなくなる（squash と同じ壊れ方）。追随すると head が変わるので、`agent-merge-verdict` は新しい head で投稿し直す（古い verdict は消さずに残す）。委任するときは「PR 作成前に `origin/main` を merge commit で取り込む」を仕様へ入れておくと、司令塔側の追随操作が不要になる。
- **エージェントがマージ操作を実行する場合は、その直前に DOCS_OPS §5「エージェントのマージ記録」が定める verdict コメントを `gh pr comment` で残す**（`merge_policy` の値を問わず MUST。残せなければマージせず human 承認へ落とす）。上記のとおり `auto-on-green` の前提条件は充足済みなので、エージェントが前提条件を自ら判定してマージする場合は `agent-merge-verdict/v1` を使う。前提条件が 1 つでも欠けて human 承認へ落ちた PR をエージェントが代行マージする場合は `agent-merge-verdict/human-v2` を使う（振り分けは `merge_policy` の値ではなくマージ判断の根拠で行う）。`gh pr merge --auto` を使う場合は `--match-head-commit <判定時 head>` を必ず付ける（DOCS_OPS §5 の MUST。このフラグを使えない `gh` では `--auto` を使わない）。
