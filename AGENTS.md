...
- **PR は base に追随させてからマージする（rebase は使わない）**。ruleset の `strict_required_status_checks_policy` が true なので、base が進むと head が古い PR はマージを拒否される（並行 PR の 2 本目以降は必ず該当する。2026-09-04 の PR #50 で "the head branch is not up to date with the base branch" を実測）。追随は `gh pr update-branch <番号>` かブランチ上での `git merge origin/main` で行う。どちらも main を merge commit で取り込むため、既存コミットの SHA は変わらない。**rebase で追随しない** — 実装コミットの SHA が変わり、`.docs/reviews/` の `verified_impl_sha` が履歴から消えて `check-evidence` の祖先判定を通らなくなる（squash と同じ壊れ方）。追随すると head が変わるので、`agent-merge-verdict` は新しい head で投稿し直す（古い verdict は消さずに残す）。
