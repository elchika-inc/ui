...
function useSidebar() {
  ...
  if (!context) {
    throw new Error("useSidebar は SidebarProvider の配下で使用してください。");
  }
...
function SidebarProvider({
  ...
  const setOpen = React.useCallback(
    (value: boolean | ((value: boolean) => boolean)) => {
      ...
      // sidebar の状態を次回表示でも維持する。
      document.cookie = `${SIDEBAR_COOKIE_NAME}=${openState}; path=/; max-age=${SIDEBAR_COOKIE_MAX_AGE}`;
    },
    [setOpenProp, open],
  );

  // viewport に応じて desktop sidebar または mobile sheet を切り替える。
  const toggleSidebar = React.useCallback(() => {
    return isMobile ? setOpenMobile((open) => !open) : setOpen((open) => !open);
  }, [isMobile, setOpen, setOpenMobile]);

  // Ctrl+B / Meta+B で sidebar を切り替える。
  React.useEffect(() => {
  ...
  // Tailwind の状態 variant から参照できる公開状態へ正規化する。
        ...
        style={
          {
            ...
            "--sidebar-width-icon-gap": `calc(${SIDEBAR_WIDTH_ICON} + 1rem)`,
            "--sidebar-width-icon-floating": `calc(${SIDEBAR_WIDTH_ICON} + 1rem + 2px)`,
            "--sidebar-width-offcanvas": `calc(${SIDEBAR_WIDTH} * -1)`,
            "--sidebar-menu-outline-shadow": "0 0 0 1px var(--sidebar-border)",
            "--sidebar-menu-outline-shadow-hover": "0 0 0 1px var(--sidebar-accent)",
            ...style,
      ...
      <Sheet open={openMobile} onOpenChange={setOpenMobile}>
        ...
        >
          <SheetHeader className="sr-only">
            ...
            <SheetDescription>モバイル用のサイドバーを表示します。</SheetDescription>
          </SheetHeader>
...
function SidebarGroupLabel({ className, render, ...props }: SidebarGroupLabelProps) {
  return useRender({
    ...
    props: mergeProps<"div">(
      {
        className: cn(
          "flex h-8 shrink-0 items-center rounded-md px-2 font-mono text-3xs font-medium tracking-label uppercase text-sidebar-foreground ring-sidebar-ring outline-hidden transition-[margin,opacity] duration-base ease-linear group-data-[collapsible=icon]:-mt-8 group-data-[collapsible=icon]:opacity-0 focus-visible:ring-3 [&>svg]:size-4 [&>svg]:shrink-0",
          className,
        ),
      },
      props,
  ...
  "peer/menu-button group/menu-button flex w-full items-center gap-2 overflow-hidden rounded-md p-2 text-left text-sm ring-sidebar-ring outline-hidden transition-[width,height,padding] group-has-data-[sidebar=menu-action]/menu-item:pr-8 group-data-[collapsible=icon]:size-8! group-data-[collapsible=icon]:p-2! hover:bg-sidebar-accent hover:text-sidebar-accent-foreground focus-visible:ring-3 active:bg-sidebar-accent active:text-sidebar-accent-foreground disabled:pointer-events-none disabled:opacity-50 aria-disabled:pointer-events-none aria-disabled:opacity-50 data-open:hover:bg-sidebar-accent data-open:hover:text-sidebar-accent-foreground data-active:bg-sidebar-accent data-active:font-medium data-active:text-sidebar-accent-foreground [&_svg]:size-4 [&_svg]:shrink-0 [&>span:last-child]:truncate",
  {
    variants: {
      variant: {
        ...
        outline:
          "bg-background shadow-(--sidebar-menu-outline-shadow) hover:bg-sidebar-accent hover:text-sidebar-accent-foreground hover:shadow-(--sidebar-menu-outline-shadow-hover)",
      },
      size: {
        default: "h-8 text-sm",
        sm: "h-7 text-xs",
...
function SidebarMenuBadge({ className, ...props }: SidebarMenuBadgeProps) {
  return (
    <div
      ...
      className={cn(
        "pointer-events-none absolute right-1 flex h-5 min-w-5 items-center justify-center rounded-md px-1 text-2xs leading-none font-medium text-sidebar-foreground tabular-nums select-none group-data-[collapsible=icon]:hidden peer-hover/menu-button:text-sidebar-accent-foreground peer-data-[size=default]/menu-button:top-1.5 peer-data-[size=lg]/menu-button:top-2.5 peer-data-[size=sm]/menu-button:top-1 peer-data-active/menu-button:text-sidebar-accent-foreground",
        className,
      )}
      {...props}
    />
