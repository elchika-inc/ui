...
function MenubarItem({ className, inset, variant = "default", ...props }: MenubarItemProps) {
  return (
    <DropdownMenuItem
      ...
      className={cn(
        "group/menubar-item gap-1.5 rounded-md px-1.5 py-1 text-sm focus:bg-accent focus:text-accent-foreground not-data-[variant=destructive]:focus:**:text-accent-foreground data-inset:pl-7 data-[variant=destructive]:text-destructive-subtle-foreground data-[variant=destructive]:focus:bg-destructive-subtle data-[variant=destructive]:focus:text-destructive-subtle-foreground data-disabled:opacity-disabled [&_svg:not([class*='size-'])]:size-4 data-[variant=destructive]:*:[svg]:text-destructive-subtle-foreground!",
        className,
      )}
      {...props}
    />
...
function MenubarShortcut({ className, ...props }: MenubarShortcutProps) {
  return (
    <DropdownMenuShortcut
      ...
      className={cn(
        "ml-auto font-mono text-2xs tracking-label text-muted-foreground group-focus/menubar-item:text-accent-foreground",
        className,
      )}
      {...props}
    />
...
function MenubarSubContent({ className, ...props }: MenubarSubContentProps) {
  return (
    <DropdownMenuSubContent
      ...
      className={cn(
        "min-w-32 rounded-lg bg-popover p-1 text-popover-foreground shadow-lg border border-border transition-[opacity,scale] duration-slow ease-entrance data-starting-style:opacity-0 data-starting-style:scale-(--motion-scale-md) data-ending-style:opacity-0 data-ending-style:scale-(--motion-scale-sm) data-ending-style:duration-fast data-instant:transition-none",
        className,
      )}
      {...props}
    />
