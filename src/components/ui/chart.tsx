...
function useChart() {
  ...
  if (!context) {
    throw new Error("useChart は <ChartContainer /> 内で使用してください");
  }
...
function ChartTooltipContent({
  ...
  return (
    <div
      className={cn(
        "grid min-w-32 items-start gap-1.5 rounded-lg border border-border/50 bg-background px-2.5 py-1.5 text-2xs shadow-lg",
        className,
      )}
    >
...
export type ChartLegendProps = React.ComponentProps<typeof RechartsPrimitive.Legend>;
    ...
    >
      {payload
        ...
        .map((item) => {
          ...
          return (
            <div
              key={`${String(item.dataKey ?? item.value ?? "value")}-${String(item.color ?? "")}`}
              className={cn(
