...
export function AppSidebar({ ...props }: React.ComponentProps<typeof Sidebar>) {
  return (
    <Sidebar {...props}>
      <SidebarContent>
        <SidebarGroup>
          <SidebarGroupLabel>変更</SidebarGroupLabel>
          <SidebarGroupContent>
            <SidebarMenu>
...
function Tree({ item }: { item: TreeItem }) {
  ...
  return (
    <SidebarMenuItem>
      ...
      >
        <SidebarMenuButton className="group/collapsible-trigger" render={<CollapsibleTrigger />}>
          <ChevronRightIcon className="transition-transform group-data-panel-open/collapsible-trigger:rotate-90" />
          <FolderIcon />
          {name}
        </SidebarMenuButton>
        <CollapsibleContent>
