...
2. catalog、light route、dark route を実ブラウザで開く。各 route で console error がなく、`preview-selectors.json` の selector が hydrated 後に1件以上存在することを確認する。catalog では、その SHA で scan された全 component が描画されていることも確認する。Provider と overlay の設計に応じた操作も行う。
...
4. 検証 route、テーマ、操作、selector の件数、console 結果、catalog で scan 由来の全 component を描画できたこと、実装 commit の40桁 SHA を新規 Markdown に記録する。
...
5. `node scripts/check-evidence.mjs` を実行する。component 固有 path が検証 SHA より新しければ証跡を作り直す。catalog / index の集約証跡と shared surface の stale 一覧は自動失敗ではないため、見た目への影響を人が確認し、必要な証跡だけを再撮影する。最新 component の証跡がその SHA の catalog 観測を兼ねるため、component 追加だけを理由に集約証跡を毎回撮り直さない。`catalog-index-r2/report.md` は一度きりの深い検証として履歴に残し、以後の hard gate 対象にしない。過去の記録は書き換えない。
