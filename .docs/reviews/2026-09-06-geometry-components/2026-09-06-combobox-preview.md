...
# combobox preview 実ブラウザ検証（幾何レイヤー）
...
- selector: `[data-slot="combobox-preview"]`。light / dark は隔離 preview route を個別に開いた。
- combobox を click した後に ArrowDown を押し、popup を表示した。 追加対象の visible を確認し、400ms待って computed style と画像を取得した。

## 実測結果

| theme | route | HTTP | selector 件数 | console 総数 / favicon 除外後 | pageerror | dark class | 横 overflow | toolbar |
|---|---|---:|---:|---:|---:|---|---|---:|
| light | `/preview/combobox/` | 200 | 1 | 0 / 0 | 0 | false | なし | 0 |
| dark | `/preview/combobox-dark/` | 200 | 1 | 0 / 0 | 0 | true | なし | 0 |

favicon.ico の404は配信資産の既知の欠落として分離した。両テーマで主文書の高さは900pxで、viewport内を JPEG 撮影した。

追加表示対象: `[data-slot="combobox-content"]`。

| theme | border-radius | border-top-width | box-shadow（生文字列） |
|---|---|---|---|
| light | `12px` | `1px` | `rgba(0, 0, 0, 0) 0px 0px 0px 0px, rgba(0, 0, 0, 0) 0px 0px 0px 0px, rgba(0, 0, 0, 0) 0px 0px 0px 0px, rgba(0, 0, 0, 0) 0px 0px 0px 0px, rgba(26, 28, 33, 0.08) 0px 4px 12px 0px` |
| dark | `12px` | `1px` | `rgba(0, 0, 0, 0) 0px 0px 0px 0px, rgba(0, 0, 0, 0) 0px 0px 0px 0px, rgba(0, 0, 0, 0) 0px 0px 0px 0px, rgba(0, 0, 0, 0) 0px 0px 0px 0px, rgba(0, 0, 0, 0.45) 0px 4px 12px 0px` |

## 証跡画像

- light: [2026-09-06-combobox-preview-light.jpg](2026-09-06-combobox-preview-light.jpg) — 1440×900px / JPEG / magic bytes `FF D8 FF`。
- dark: [2026-09-06-combobox-preview-dark.jpg](2026-09-06-combobox-preview-dark.jpg) — 1440×900px / JPEG / magic bytes `FF D8 FF`。
