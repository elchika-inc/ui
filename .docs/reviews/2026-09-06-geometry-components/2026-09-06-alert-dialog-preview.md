...
# alert-dialog preview 実ブラウザ検証（幾何レイヤー）
...
- selector: `[data-slot="alert-dialog-content"]`。light / dark は隔離 preview route を個別に開いた。

## 実測結果

| theme | route | HTTP | selector 件数 | console 総数 / favicon 除外後 | pageerror | dark class | 横 overflow | toolbar |
|---|---|---:|---:|---:|---:|---|---|---:|
| light | `/preview/alert-dialog/` | 200 | 1 | 0 / 0 | 0 | false | なし | 0 |
| dark | `/preview/alert-dialog-dark/` | 200 | 1 | 0 / 0 | 0 | true | なし | 0 |

favicon.ico の404は配信資産の既知の欠落として分離した。両テーマで主文書の高さは900pxで、viewport内を JPEG 撮影した。

selector先頭の computed style を取得した（Select は popup 面の測定）。

| theme | height | border-radius | transition-duration | box-shadow（生文字列） |
|---|---|---|---|---|
| light | `173px` | `12px` | `0.1s` | `rgba(0, 0, 0, 0) 0px 0px 0px 0px, rgba(0, 0, 0, 0) 0px 0px 0px 0px, rgba(0, 0, 0, 0) 0px 0px 0px 0px, rgba(0, 0, 0, 0) 0px 0px 0px 0px, rgba(26, 28, 33, 0.1) 0px 12px 32px 0px` |
| dark | `173px` | `12px` | `0.1s` | `rgba(0, 0, 0, 0) 0px 0px 0px 0px, rgba(0, 0, 0, 0) 0px 0px 0px 0px, rgba(0, 0, 0, 0) 0px 0px 0px 0px, rgba(0, 0, 0, 0) 0px 0px 0px 0px, rgba(0, 0, 0, 0.55) 0px 12px 32px 0px` |

## 証跡画像

- light: [2026-09-06-alert-dialog-preview-light.jpg](2026-09-06-alert-dialog-preview-light.jpg) — 1440×900px / JPEG / magic bytes `FF D8 FF`。
- dark: [2026-09-06-alert-dialog-preview-dark.jpg](2026-09-06-alert-dialog-preview-dark.jpg) — 1440×900px / JPEG / magic bytes `FF D8 FF`。
