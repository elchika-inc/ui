...
- selector: `[data-slot="toast-preview"]`。light / dark は隔離 preview route を個別に開いた。
- `[data-slot="toast-action-add"]` を click し、timeout 0 の Action 付き通知を表示した。 追加対象の visible を確認し、400ms待って computed style と画像を取得した。

## 実測結果

| theme | route | HTTP | selector 件数 | console 総数 / favicon 除外後 | pageerror | dark class | 横 overflow | toolbar |
|---|---|---:|---:|---:|---:|---|---|---:|
| light | `/preview/toast/` | 200 | 1 | 0 / 0 | 0 | false | なし | 0 |
| dark | `/preview/toast-dark/` | 200 | 1 | 0 / 0 | 0 | true | なし | 0 |

favicon.ico の404は配信資産の既知の欠落として分離した。両テーマで主文書の高さは900pxで、viewport内を JPEG 撮影した。

追加表示対象: `[data-slot="toast"]`。
...
- light: [2026-09-06-toast-preview-light.jpg](2026-09-06-toast-preview-light.jpg) — 1440×900px / JPEG / magic bytes `FF D8 FF`。
- dark: [2026-09-06-toast-preview-dark.jpg](2026-09-06-toast-preview-dark.jpg) — 1440×900px / JPEG / magic bytes `FF D8 FF`。
