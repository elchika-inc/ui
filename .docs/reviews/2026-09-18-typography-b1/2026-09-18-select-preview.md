...
# select preview 実ブラウザ検証（組版と Mono）

## 成功基準と対象

成功基準は [委任仕様 §4・§B](../../plans/2026-09-17-typography-layer.md) と司令塔の追加裁定。group label を text-2xs にした。
...
| --- | --- | --- | --- | --- | --- | --- | --- |
| light | `/preview/select/` | 200 | 2 | 0 | 0 | 0 | false |
| dark | `/preview/select-dark/` | 200 | 2 | 0 | 0 | 0 | true |
...
| --- | --- | --- | --- | --- | --- | --- | --- |
| light | `select-label` / 利用可能 | 12px | 18px | 0.32px | 400 | normal | 26 / 26 |
| light | `select-label` / 準備中 | 12px | 18px | 0.32px | 400 | normal | 26 / 26 |
| dark | `select-label` / 利用可能 | 12px | 18px | 0.32px | 400 | normal | 26 / 26 |
| dark | `select-label` / 準備中 | 12px | 18px | 0.32px | 400 | normal | 26 / 26 |

computed fontFamily（light / dark で一致）:

- 利用可能 / 準備中: `"IBM Plex Sans JP", "IBM Plex Sans", system-ui, sans-serif`。

2つのgroup labelはDOM上で測定した。画像ではスクロール領域の先頭を表示し、下端の選択肢が一部隠れるのは既存の `max-h-32` によるスクロール表示である。

## 画像と判定範囲

- light: [2026-09-18-select-preview-light.jpg](2026-09-18-select-preview-light.jpg)。
- dark: [2026-09-18-select-preview-dark.jpg](2026-09-18-select-preview-dark.jpg)。
