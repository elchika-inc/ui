...
# context-menu preview 実ブラウザ検証（組版と Mono）

## 成功基準と対象

成功基準は [委任仕様 §4・§B](../../plans/2026-09-17-typography-layer.md) と司令塔の追加裁定。group label を text-2xs、shortcut を font-mono text-2xs tracking-label にした。司令塔裁定で preview の既存項目に shortcut の最小例 ⌘K を追加した。
...
```css
[data-slot="context-menu-label"], [data-slot="context-menu-shortcut"]
```

`[data-slot="context-menu-trigger"]` を右クリックして popup を開いた。

## 表示とエラー

| theme | route | HTTP | 対象数 | console error（favicon除外） | favicon error | pageerror | dark class |
| --- | --- | --- | --- | --- | --- | --- | --- |
| light | `/preview/context-menu/` | 200 | 3 | 0 | 0 | 0 | false |
| dark | `/preview/context-menu-dark/` | 200 | 3 | 0 | 0 | 0 | true |
...
| --- | --- | --- | --- | --- | --- | --- | --- |
| light | `context-menu-label` / ファイル | 12px | 18px | 0.32px | 500 | normal | 26 / 26 |
| light | `context-menu-shortcut` / ⌘K | 12px | 18px | 0.96px | 400 | normal | 18 / 18 |
| light | `context-menu-label` / 並び順 | 12px | 18px | 0.32px | 500 | normal | 26 / 26 |
| dark | `context-menu-label` / ファイル | 12px | 18px | 0.32px | 500 | normal | 26 / 26 |
| dark | `context-menu-shortcut` / ⌘K | 12px | 18px | 0.96px | 400 | normal | 18 / 18 |
| dark | `context-menu-label` / 並び順 | 12px | 18px | 0.32px | 500 | normal | 26 / 26 |

computed fontFamily（light / dark で一致）:

- ファイル / 並び順: `"IBM Plex Sans JP", "IBM Plex Sans", system-ui, sans-serif`。
- ⌘K: `"IBM Plex Mono", "IBM Plex Sans JP", ui-monospace, monospace`。

## 画像と判定範囲

- light: [2026-09-18-context-menu-preview-light.jpg](2026-09-18-context-menu-preview-light.jpg)。
- dark: [2026-09-18-context-menu-preview-dark.jpg](2026-09-18-context-menu-preview-dark.jpg)。
