...
# kbd preview 実ブラウザ検証（組版と Mono）

## 成功基準と対象

成功基準は [委任仕様 §4・§B](../../plans/2026-09-17-typography-layer.md) と司令塔の追加裁定。font-sans text-xs を font-mono text-2xs に置換し、固定高さのため leading-none を追加した。
...
| --- | --- | --- | --- | --- | --- | --- | --- |
| light | `/preview/kbd/` | 200 | 3 | 0 | 0 | 0 | false |
| dark | `/preview/kbd-dark/` | 200 | 3 | 0 | 0 | 0 | true |
...
| --- | --- | --- | --- | --- | --- | --- | --- |
| light | `kbd` / Esc | 12px | 12px | 0.32px | 500 | normal | 20 / 20 |
| light | `kbd` / ⌘ | 12px | 12px | 0.32px | 500 | normal | 20 / 20 |
| light | `kbd` / K | 12px | 12px | 0.32px | 500 | normal | 20 / 20 |
| dark | `kbd` / Esc | 12px | 12px | 0.32px | 500 | normal | 20 / 20 |
| dark | `kbd` / ⌘ | 12px | 12px | 0.32px | 500 | normal | 20 / 20 |
| dark | `kbd` / K | 12px | 12px | 0.32px | 500 | normal | 20 / 20 |

computed fontFamily（light / dark で一致）:

- Esc / ⌘ / K: `"IBM Plex Mono", "IBM Plex Sans JP", ui-monospace, monospace`。

固定高さの溢れ検査は light 3/3・dark 3/3 が `20 <= 20` で通過した。Kbd は数値要素ではないため `fontVariantNumeric: normal` を記録し、tabular-nums の合格条件は Badge の numeric 例へ適用した。

## 画像と判定範囲

- light: [2026-09-18-kbd-preview-light.jpg](2026-09-18-kbd-preview-light.jpg)。
- dark: [2026-09-18-kbd-preview-dark.jpg](2026-09-18-kbd-preview-dark.jpg)。
