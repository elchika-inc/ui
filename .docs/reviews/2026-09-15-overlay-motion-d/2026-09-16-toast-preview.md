...
# toast preview 実ブラウザ検証（overlay モーション統一 PR D）
...
- preview selector は `preview-selectors.json` の `[data-slot="toast-preview"]`、実測対象は `[data-slot="toast"]`。
- 操作: 「Action付き通知」をクリックして開き、「通知を閉じる」で閉じる。timeout=0 の既存 preview 操作を利用した。
...
|---|---|---:|---:|---:|---:|---:|---|---|
| light | `/preview/toast/` | 200 | 1 | 0 | 0 | 0 | false | なし |
| dark | `/preview/toast-dark/` | 200 | 1 | 0 | 0 | 0 | true | なし |
...
|---|---|---|---|---|---:|---:|---|---|
| light | `0.4s` | `cubic-bezier(0.16, 1, 0.3, 1)` | `transform, opacity, height, filter` | `78px` | 27 | 425.400 | 未観測（best-effort） | なし（false / null） |
| dark | `0.4s` | `cubic-bezier(0.16, 1, 0.3, 1)` | `transform, opacity, height, filter` | `78px` | 27 | 428.800 | 未観測（best-effort） | なし（false / null） |

close の宣言時間・曲線も open と同値。全テーマで ending 属性の観測後に DOM が消えた。上表の観測時間は16msポーリングの実測であり、CSS宣言時間とは区別する。

通常状態の filter は両テーマとも `none`。close のポーリングでは `blur(...)` の増加を観測した。duration は CSSOM が共通値 `0.4s` 一つで返す等価表記で、4プロパティすべてに適用される。starting 属性は MutationObserver の読み取りでは捕捉できなかったため、観測したとは記録しない。

## 証跡画像

- light: [2026-09-16-toast-preview-light.jpg](2026-09-16-toast-preview-light.jpg)（1440×900 / JPEG）。
- dark: [2026-09-16-toast-preview-dark.jpg](2026-09-16-toast-preview-dark.jpg)（1440×900 / JPEG）。
