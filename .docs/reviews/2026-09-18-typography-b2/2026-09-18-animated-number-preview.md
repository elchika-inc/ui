...
# animated-number preview 実ブラウザ検証（組版と Mono）
...
- preview selector: `[data-slot="animated-number-preview"]`。

初期値12,000と120.00の2要素を測定した。数値補間のロジックは変更していない。

## 表示とエラー

| theme | route | HTTP | selector数 | lang | dark class | console error（favicon除外） | favicon 404 | pageerror | scrollWidth / clientWidth |
|---|---|---|---|---|---|---|---|---|---|
| light | `/preview/animated-number/` | 200 | 1 | ja | false | 0 | 0 | 0 | 1440 / 1440 |
| dark | `/preview/animated-number-dark/` | 200 | 1 | ja | true | 0 | 0 | 0 | 1440 / 1440 |
...
|---|---|---|---|---|---|---|---|---|
| light | animated-number | "IBM Plex Mono", "IBM Plex Sans JP", ui-monospace, monospace | 30px | 40.5px | 0.32px | tabular-nums | start | 1 |
| light | animated-number | "IBM Plex Mono", "IBM Plex Sans JP", ui-monospace, monospace | 21px | 28.35px | 0.32px | tabular-nums | start | 1 |
| dark | animated-number | "IBM Plex Mono", "IBM Plex Sans JP", ui-monospace, monospace | 30px | 40.5px | 0.32px | tabular-nums | start | 1 |
| dark | animated-number | "IBM Plex Mono", "IBM Plex Sans JP", ui-monospace, monospace | 21px | 28.35px | 0.32px | tabular-nums | start | 1 |

全対象の fontFamily が `"IBM Plex Mono"` で始まる。数値要素は全件 `tabular-nums`。SidebarGroupLabel の `normal` は数値要素ではないため対象外。

## 証跡画像と判定範囲

- light: [2026-09-18-animated-number-preview-light.jpg](2026-09-18-animated-number-preview-light.jpg) — 1440×900 JPEG、magic bytes `FF D8 FF`。
- dark: [2026-09-18-animated-number-preview-dark.jpg](2026-09-18-animated-number-preview-dark.jpg) — 1440×900 JPEG、magic bytes `FF D8 FF`。
