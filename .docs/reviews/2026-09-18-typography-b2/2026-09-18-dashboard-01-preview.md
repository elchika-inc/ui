...
# dashboard-01 preview 実ブラウザ検証（組版と Mono）
...
- preview selector: `[data-slot="dashboard-01-preview"]`。

CardTitle 4要素の Mono と tabular-nums、SidebarGroupLabel 1要素、ナビゲーションの12px注釈を測定した。

## 表示とエラー

| theme | route | HTTP | selector数 | lang | dark class | console error（favicon除外） | favicon 404 | pageerror | scrollWidth / clientWidth |
|---|---|---|---|---|---|---|---|---|---|
| light | `/preview/dashboard-01/` | 200 | 1 | ja | false | 0 | 0 | 0 | 1440 / 1440 |
| dark | `/preview/dashboard-01-dark/` | 200 | 1 | ja | true | 0 | 0 | 0 | 1440 / 1440 |
...
| light | card-title | "IBM Plex Mono", "IBM Plex Sans JP", ui-monospace, monospace | 25px | 33.75px | 0.32px | tabular-nums | start | 4 |
| dark | sidebar-group-label | "IBM Plex Mono", "IBM Plex Sans JP", ui-monospace, monospace | 11px | 16.5px | 0.88px | normal | start | 1 |
| dark | card-title | "IBM Plex Mono", "IBM Plex Sans JP", ui-monospace, monospace | 25px | 33.75px | 0.32px | tabular-nums | start | 4 |

全対象の fontFamily が `"IBM Plex Mono"` で始まる。数値要素は全件 `tabular-nums`。SidebarGroupLabel の `normal` は数値要素ではないため対象外。

## 証跡画像と判定範囲

- light: [2026-09-18-dashboard-01-preview-light.jpg](2026-09-18-dashboard-01-preview-light.jpg) — 1440×900 JPEG、magic bytes `FF D8 FF`。
- dark: [2026-09-18-dashboard-01-preview-dark.jpg](2026-09-18-dashboard-01-preview-dark.jpg) — 1440×900 JPEG、magic bytes `FF D8 FF`。
