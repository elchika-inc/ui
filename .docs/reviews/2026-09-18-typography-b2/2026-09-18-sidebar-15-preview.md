...
# sidebar-15 preview 実ブラウザ検証（組版と Mono）
...
- preview selector: `[data-slot="sidebar-15-preview"]`。

先頭のチーム切替ボタン（menu-button / aria-haspopup=menu）を開き、チームラベルとメール・カレンダー曜日の12px表示を測定した。

## 表示とエラー

| theme | route | HTTP | selector数 | lang | dark class | console error（favicon除外） | favicon 404 | pageerror | scrollWidth / clientWidth |
|---|---|---|---|---|---|---|---|---|---|
| light | `/preview/sidebar-15/` | 200 | 1 | ja | false | 0 | 0 | 0 | 1440 / 1440 |
| dark | `/preview/sidebar-15-dark/` | 200 | 1 | ja | true | 0 | 0 | 0 | 1440 / 1440 |
...
| light | dropdown-menu-label | 12px | 18px | 0.32px | 1 |
| dark | SPAN | 12px | 18px | 0.32px | 1 |
| dark | TH | 12px | 18px | 0.32px | 7 |
| dark | dropdown-menu-label | 12px | 18px | 0.32px | 1 |

## Mono と数値の computed style

| theme | 対象 | fontFamily | fontSize | lineHeight | letterSpacing | fontVariantNumeric | textAlign | 件数 |
|---|---|---|---|---|---|---|---|---|
| light | sidebar-group-label | "IBM Plex Mono", "IBM Plex Sans JP", ui-monospace, monospace | 11px | 16.5px | 0.88px | normal | start | 2 |
| light | sidebar-group-label | "IBM Plex Mono", "IBM Plex Sans JP", ui-monospace, monospace | 14px | 21px | 1.12px | normal | left | 3 |
| dark | sidebar-group-label | "IBM Plex Mono", "IBM Plex Sans JP", ui-monospace, monospace | 11px | 16.5px | 0.88px | normal | start | 2 |
| dark | sidebar-group-label | "IBM Plex Mono", "IBM Plex Sans JP", ui-monospace, monospace | 14px | 21px | 1.12px | normal | left | 3 |

全対象の fontFamily が `"IBM Plex Mono"` で始まる。数値要素は全件 `tabular-nums`。SidebarGroupLabel の `normal` は数値要素ではないため対象外。

## 証跡画像と判定範囲

- light: [2026-09-18-sidebar-15-preview-light.jpg](2026-09-18-sidebar-15-preview-light.jpg) — 1440×900 JPEG、magic bytes `FF D8 FF`。
- dark: [2026-09-18-sidebar-15-preview-dark.jpg](2026-09-18-sidebar-15-preview-dark.jpg) — 1440×900 JPEG、magic bytes `FF D8 FF`。
