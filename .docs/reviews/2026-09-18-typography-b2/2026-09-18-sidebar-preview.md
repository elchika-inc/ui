...
# sidebar preview 実ブラウザ検証（組版と Mono）
...
- preview selector: `[data-slot="sidebar-preview"]`。

展開状態の SidebarGroupLabel と SidebarMenuBadge を測定した。SidebarMenuButton size sm と SidebarMenuSubButton の variant 付き text-xs は13px役割として維持する（後者は司令塔裁定）。

## 表示とエラー

| theme | route | HTTP | selector数 | lang | dark class | console error（favicon除外） | favicon 404 | pageerror | scrollWidth / clientWidth |
|---|---|---|---|---|---|---|---|---|---|
| light | `/preview/sidebar/` | 200 | 1 | ja | false | 0 | 0 | 0 | 1440 / 1440 |
| dark | `/preview/sidebar-dark/` | 200 | 1 | ja | true | 0 | 0 | 0 | 1440 / 1440 |
...
この preview のラベルは和文のみ。Google Fonts の unicode-range 分割により Latin 用の Mono face が要求されず、両テーマで load 前 false、HTTP 200 の明示 load 後 true を観測した。
...
|---|---|---|---|---|---|
| light | sidebar-menu-badge | 12px | 12px | 0.32px | 1 |
| light | SPAN | 12px | 18px | 0.32px | 1 |
| dark | sidebar-menu-badge | 12px | 12px | 0.32px | 1 |
| dark | SPAN | 12px | 18px | 0.32px | 1 |

## Mono と数値の computed style

| theme | 対象 | fontFamily | fontSize | lineHeight | letterSpacing | fontVariantNumeric | textAlign | 件数 |
|---|---|---|---|---|---|---|---|---|
| light | sidebar-group-label | "IBM Plex Mono", "IBM Plex Sans JP", ui-monospace, monospace | 11px | 16.5px | 0.88px | normal | start | 1 |
| dark | sidebar-group-label | "IBM Plex Mono", "IBM Plex Sans JP", ui-monospace, monospace | 11px | 16.5px | 0.88px | normal | start | 1 |

全対象の fontFamily が `"IBM Plex Mono"` で始まる。数値要素は全件 `tabular-nums`。SidebarGroupLabel の `normal` は数値要素ではないため対象外。

## 固定高さの溢れ検査（司令塔追加指示）

| theme | 対象 | fontSize | lineHeight | scrollHeight | clientHeight | 判定 |
|---|---|---|---|---|---|---|
| light | sidebar-group-label | 11px | 16.5px | 32 | 32 | PASS |
| light | sidebar-menu-badge | 12px | 12px | 20 | 20 | PASS |
| dark | sidebar-group-label | 11px | 16.5px | 32 | 32 | PASS |
| dark | sidebar-menu-badge | 12px | 12px | 20 | 20 | PASS |

SidebarMenuBadge は `text-2xs` を維持し `leading-none` を追加。12px / line-height 12px、20px の固定高さに収まり、両テーマで溢れ0。SidebarGroupLabel は11px / 16.5px、32px内に収まる。

## 証跡画像と判定範囲

- light: [2026-09-18-sidebar-preview-light.jpg](2026-09-18-sidebar-preview-light.jpg) — 1440×900 JPEG、magic bytes `FF D8 FF`。
- dark: [2026-09-18-sidebar-preview-dark.jpg](2026-09-18-sidebar-preview-dark.jpg) — 1440×900 JPEG、magic bytes `FF D8 FF`。
