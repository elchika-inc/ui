...
# table preview 実ブラウザ検証（組版と Mono）
...
- preview selector: `[data-slot="table-preview"]`。

table-container を右端まで横スクロールし、金額見出しと本文2セル・合計1セルを撮影・測定した。左側の商品名が画像で切れているのは既存の局所横スクロール容器の表示範囲による。numeric 未指定の注文番号・商品は Mono を追加していない。

## 表示とエラー

| theme | route | HTTP | selector数 | lang | dark class | console error（favicon除外） | favicon 404 | pageerror | scrollWidth / clientWidth |
|---|---|---|---|---|---|---|---|---|---|
| light | `/preview/table/` | 200 | 1 | ja | false | 0 | 0 | 0 | 1440 / 1440 |
| dark | `/preview/table-dark/` | 200 | 1 | ja | true | 0 | 0 | 0 | 1440 / 1440 |
...
この preview に text-2xs の測定対象はない。数値の実測は次節に記録する。

## Mono と数値の computed style

| theme | 対象 | fontFamily | fontSize | lineHeight | letterSpacing | fontVariantNumeric | textAlign | 件数 |
|---|---|---|---|---|---|---|---|---|
| light | table-head | "IBM Plex Mono", "IBM Plex Sans JP", ui-monospace, monospace | 14px | 21px | 0.32px | tabular-nums | right | 1 |
| light | table-cell | "IBM Plex Mono", "IBM Plex Sans JP", ui-monospace, monospace | 14px | 21px | 0.32px | tabular-nums | right | 3 |
| dark | table-head | "IBM Plex Mono", "IBM Plex Sans JP", ui-monospace, monospace | 14px | 21px | 0.32px | tabular-nums | right | 1 |
| dark | table-cell | "IBM Plex Mono", "IBM Plex Sans JP", ui-monospace, monospace | 14px | 21px | 0.32px | tabular-nums | right | 3 |

全対象の fontFamily が `"IBM Plex Mono"` で始まる。数値要素は全件 `tabular-nums`。SidebarGroupLabel の `normal` は数値要素ではないため対象外。

`numeric=true / false / undefined` を TableHead / TableCell それぞれ renderToStaticMarkup で別途確認した（6ケース、exit 0）。true のときだけ `data-numeric=""` を出し、生の numeric 属性を DOM へ流さず、className と data-probe の転送を維持した。

## 証跡画像と判定範囲

- light: [2026-09-18-table-preview-light.jpg](2026-09-18-table-preview-light.jpg) — 1440×900 JPEG、magic bytes `FF D8 FF`。
- dark: [2026-09-18-table-preview-dark.jpg](2026-09-18-table-preview-dark.jpg) — 1440×900 JPEG、magic bytes `FF D8 FF`。
